$(function () {
  ...
  let defaults = {
    ...
    labelMeanTemperature: 'средна t',
    labelTotalPrecipitation: 'валеж',
    labelAplitudeTemperature: 't аплитуда',
    data: null,
    saveImageAs: 'PNG',
  ...
  $.getJSON('./minimal_schema.json', function (result) {
    $(DOMform).empty();
  ...
  function extractValues () {
    let values = $.extend({}, defaults, editor.getValue());
  ...
  function buildTable () {
    return new Handsontable(DOMtable, {
      ...
      rowHeaders: ['P', 't° (ср.)', 't° (мин.)', 't° (макс.)'],
      editor: 'numeric',
      default: 0,
      minCols: NUM_COLS,
      maxCols: NUM_COLS,
